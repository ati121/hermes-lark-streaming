...
class TestInteractiveDeviceTextSizeTransport:
    @staticmethod
    def _enable_device_sizes(ctrl: StreamCardController) -> None:
        ctrl._cfg._raw["hermes_lark_streaming"].update({
            "text_sizes": {
                "body": {"default": "large", "pc": "normal", "mobile": "large"},
                "footer": {"default": "small", "pc": "x-small", "mobile": "small"},
            },
            "footer": {
                "fields": [["status", "elapsed", "model", "tokens", "api_calls"]],
                "show_label": False,
            },
        })

    @pytest.mark.asyncio
    async def test_create_uses_full_interactive_card(self) -> None:
        ctrl = _setup_ctrl(linear=True)
        self._enable_device_sizes(ctrl)
        session = _make_session("msg_interactive_create")

        await ctrl._do_create_linear_card(session)

        assert session.interactive_mode is True
        assert session.card_id == "im:msg_id_reply"
        assert session.card_msg_id == "msg_id_reply"
        ctrl._client.reply_card.assert_called_once()
    ...
    async def test_flush_replaces_whole_interactive_card(self) -> None:
        ctrl = _setup_ctrl(linear=True)
        self._enable_device_sizes(ctrl)
        session = _make_session("msg_interactive_flush")
        await ctrl._do_create_linear_card(session)
        assert session.unified_state is not None
        session.unified_state.on_answer_delta("手机端大字号正文")
        ...
        message_id, card = ctrl._client.update_card.call_args.args
        assert message_id == "msg_id_reply"
        answer = next(
            element for element in card["body"]["elements"]
            if element.get("element_id") == ANSWER_ELEMENT_ID
        )
        assert answer["content"] == "手机端大字号正文"
        assert answer["text_size"] == "hls_body"

    @pytest.mark.asyncio
    async def test_flush_keeps_tokens_arriving_during_patch_dirty(self) -> None:
        ctrl = _setup_ctrl(linear=True)
        self._enable_device_sizes(ctrl)
        session = _make_session("msg_interactive_race")
        await ctrl._do_create_linear_card(session)
        assert session.unified_state is not None
        ...
        ctrl._client.update_card = AsyncMock(side_effect=receive_more_text)
        with patch.object(ctrl, "_schedule_linear_flush") as schedule:
            await ctrl._do_unified_flush(session)
    ...
    async def test_complete_replaces_card_and_keeps_footer_rules(self) -> None:
        ctrl = _setup_ctrl(linear=True)
        self._enable_device_sizes(ctrl)
        ctrl._release_session_data = lambda session: None
        session = _make_session("msg_interactive_complete")
        await ctrl._do_create_linear_card(session)
        assert session.unified_state is not None
        session.unified_state.on_answer_delta("最终答案")
        ...
        card = ctrl._client.update_card.call_args.args[1]
        footer = next(
            element for element in card["body"]["elements"]
            if element.get("text_size") == "hls_footer"
        )
        assert "grok-4.5" in footer["content"]
        assert "↑ 643.1K ↓ 1.5K" in footer["content"]
        assert "API 2" in footer["content"]
        assert "Model" not in footer["content"]
...
class TestProfileScopedController:
    """multiplex: 控制器按 profile home 隔离配置与凭据."""

    def test_controller_binds_profile_home_and_config(self, tmp_path) -> None:
        ...
        ctrl = StreamCardController(home)
        assert ctrl._profile_home == home.resolve()
        assert ctrl._cfg.feishu_app_id == "cli_from_x"

    def test_two_profiles_get_different_credentials(self, tmp_path) -> None:
        home_a = tmp_path / "a"
        home_b = tmp_path / "b"
        for home, app_id in ((home_a, "cli_a"), (home_b, "cli_b")):
            home.mkdir()
            (home / "config.yaml").write_text(
                "feishu:\n  app_id: " + app_id + "\n  app_secret: s\n",
                encoding="utf-8",
            )
        ctrl_a = StreamCardController(home_a)
        ctrl_b = StreamCardController(home_b)
        assert ctrl_a._cfg.feishu_app_id == "cli_a"
        assert ctrl_b._cfg.feishu_app_id == "cli_b"

    def test_get_controller_is_per_home(self, tmp_path) -> None:
        from hermes_lark_streaming.controller import get_controller, reset_controllers
        ...
        home_b.mkdir()
        try:
            ctrl_a = get_controller(home_a)
            ctrl_b = get_controller(home_b)
            assert ctrl_a is not ctrl_b
            assert get_controller(home_a) is ctrl_a  # 同一 home 复用实例
        finally:
            reset_controllers()

    def test_get_controller_defaults_to_current_home(self) -> None:
        ...
        try:
            ctrl = get_controller()
            assert ctrl._profile_home == hermes_home().resolve()
        finally:
            reset_controllers()


class TestCredentialScope:
    """enabled / _ensure_init 在 multiplex 无 scope 时自建 profile scope."""

    def _fake_secret_scope(self, monkeypatch, *, multiplex: bool, scope):
        import sys
        import types

        state = {"active": multiplex, "scope": scope}

        class _Unscoped(RuntimeError):
            pass

        fake = types.ModuleType("agent.secret_scope")
        fake.is_multiplex_active = lambda: state["active"]
        fake.current_secret_scope = lambda: state["scope"]
        fake.set_secret_scope = lambda secrets: ("token", secrets)
        fake.reset_secret_scope = lambda token: state.__setitem__("scope", scope)
        fake.build_profile_secret_scope = lambda home: {"FEISHU_APP_ID": "scoped_id", "FEISHU_APP_SECRET": "scoped_secret"}
        fake.UnscopedSecretError = _Unscoped
        monkeypatch.setitem(sys.modules, "agent.secret_scope", fake)
        return state

    def test_needs_fallback_scope_true_when_multiplex_without_scope(self, monkeypatch, tmp_path) -> None:
        self._fake_secret_scope(monkeypatch, multiplex=True, scope=None)
        ctrl = StreamCardController(tmp_path)
        assert ctrl._needs_fallback_scope() is True

    def test_needs_fallback_scope_false_when_scope_present(self, monkeypatch, tmp_path) -> None:
        self._fake_secret_scope(monkeypatch, multiplex=True, scope={"A": "b"})
        ctrl = StreamCardController(tmp_path)
        assert ctrl._needs_fallback_scope() is False

    def test_needs_fallback_scope_false_when_multiplex_off(self, monkeypatch, tmp_path) -> None:
        self._fake_secret_scope(monkeypatch, multiplex=False, scope=None)
        ctrl = StreamCardController(tmp_path)
        assert ctrl._needs_fallback_scope() is False

    def test_needs_fallback_scope_false_without_host_api(self, monkeypatch, tmp_path) -> None:
        import sys
        monkeypatch.setitem(sys.modules, "agent.secret_scope", None)
        ctrl = StreamCardController(tmp_path)
        assert ctrl._needs_fallback_scope() is False

    def test_credential_scope_installs_and_restores(self, monkeypatch, tmp_path) -> None:
        state = self._fake_secret_scope(monkeypatch, multiplex=True, scope=None)
        ctrl = StreamCardController(tmp_path)
        assert state["scope"] is None
        with ctrl._credential_scope():
            assert state["scope"] == {"FEISHU_APP_ID": "scoped_id", "FEISHU_APP_SECRET": "scoped_secret"}
        assert state["scope"] is None
        assert ctrl._unscoped_enabled is None  # 未触碰 enabled 缓存

    def test_credential_scope_is_noop_when_multiplex_off(self, monkeypatch, tmp_path) -> None:
        state = self._fake_secret_scope(monkeypatch, multiplex=False, scope=None)
        ctrl = StreamCardController(tmp_path)
        with ctrl._credential_scope():
            assert state["scope"] is None

    def test_credential_scope_never_clobbers_existing_scope(self, monkeypatch, tmp_path) -> None:
        existing = {"FEISHU_APP_ID": "outer"}
        state = self._fake_secret_scope(monkeypatch, multiplex=True, scope=existing)
        ctrl = StreamCardController(tmp_path)
        with ctrl._credential_scope():
            assert state["scope"] is existing
        assert state["scope"] is existing

    def test_enabled_reads_credentials_from_scope(self, monkeypatch, tmp_path) -> None:
        """scope 里有凭据就算 enabled —— 即使 os.environ/config.yaml 都没有."""
        import sys
        import types

        self._fake_secret_scope(monkeypatch, multiplex=True, scope=None)
        # hermes_constants 不存在 → _get_secret 回退 os.environ; 这里让它空
        ...
        home.mkdir()
        (home / "config.yaml").write_text("hermes_lark_streaming:\n  enabled: true\n", encoding="utf-8")

        ctrl = StreamCardController(home)
        # 让 _get_secret 走 scope: 宿主 get_secret 由 fake 提供
        fake_reads = {"value": ""}

        def _get_secret(name, default=None):
            return {"FEISHU_APP_ID": "scoped_id", "FEISHU_APP_SECRET": "s"}.get(name, default)

        from hermes_lark_streaming.config import reader as reader_mod
        monkeypatch.setattr(reader_mod, "_get_secret", _get_secret)

        assert ctrl.enabled is True
        assert ctrl._unscoped_enabled is True

    def test_enabled_false_and_not_cached_when_credentials_missing(self, monkeypatch, tmp_path) -> None:
        self._fake_secret_scope(monkeypatch, multiplex=True, scope=None)
        monkeypatch.setitem(sys.modules, "hermes_constants", None)
        from hermes_lark_streaming.config import reader as reader_mod
        monkeypatch.setattr(reader_mod, "_get_secret", lambda name, default=None: default)
        ...
        # 负结果不缓存: 稍后 .env 就绪时下一次访问仍会重新判断
        assert ctrl._unscoped_enabled is None

    @pytest.mark.asyncio
    async def test_ensure_init_logs_profile_home(self, monkeypatch, tmp_path, caplog) -> None:
        self._fake_secret_scope(monkeypatch, multiplex=True, scope=None)
        monkeypatch.setitem(sys.modules, "hermes_constants", None)
        from hermes_lark_streaming.config import reader as reader_mod

        def _get_secret(name, default=None):
            return {"FEISHU_APP_ID": "cli_scoped", "FEISHU_APP_SECRET": "s"}.get(name, default)
...
class TestCredentialScope:
    ...
    def _fake_secret_scope(self, monkeypatch, *, multiplex: bool, scope):
        ...
        def _set_scope(secrets):
            state["scope"] = secrets
            return ("token", secrets)

        fake.set_secret_scope = _set_scope
        fake.reset_secret_scope = lambda token: state.__setitem__("scope", original)
        fake.build_profile_secret_scope = lambda home: {
            "FEISHU_APP_ID": "scoped_id",
            "FEISHU_APP_SECRET": "scoped_secret",
        }
        fake.UnscopedSecretError = type("UnscopedSecretError", (RuntimeError,), {})
        monkeypatch.setitem(sys.modules, "agent.secret_scope", fake)
        return state
    ...
    def test_enabled_true_when_scope_has_credentials(self, monkeypatch, tmp_path) -> None:
        """scope 里有凭据就算 enabled —— 即使 config.yaml/env 都没有."""
        self._fake_secret_scope(monkeypatch, multiplex=True, scope=None)
        monkeypatch.setitem(sys.modules, "hermes_constants", None)
    ...
    def test_enabled_false_and_not_cached_when_credentials_missing(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr(reader_mod, "_get_secret", lambda name, default="": default)
    ...
    @pytest.mark.asyncio
    async def test_ensure_init_uses_scope_and_logs_profile_home(self, monkeypatch, tmp_path, caplog) -> None:
        self._fake_secret_scope(monkeypatch, multiplex=True, scope=None)
        monkeypatch.setitem(sys.modules, "hermes_constants", None)
        ...
        monkeypatch.setattr("hermes_lark_streaming.controller.core.FeishuClient", _FakeClient)
        with caplog.at_level("INFO", logger="hermes_lark_streaming"):
