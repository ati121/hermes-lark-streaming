...
from hermes_lark_streaming.config import (
    Config,
...
class TestTextSizes:
    def test_scalar_and_device_mapping(self) -> None:
        assert normalize_text_sizes({
            "body": "large",
            "footer": {
                "default": "x-small",
                "pc": "small",
    ...
    def test_pc_only_mapping_uses_role_default_elsewhere(self) -> None:
    ...
    @pytest.mark.parametrize(
        ("value", "path"),
        [
            ({"unknown": "normal"}, "hermes_lark_streaming.text_sizes.unknown"),
            ({"body": "normal_v2"}, "hermes_lark_streaming.text_sizes.body"),
            ({"footer": {"tablet": "small"}}, "hermes_lark_streaming.text_sizes.footer.tablet"),
            ({"footer": {"mobile": 12}}, "hermes_lark_streaming.text_sizes.footer.mobile"),
        ],
    )
    def test_invalid_values_include_precise_path(
        self,
        value: object,
        path: str,
    ) -> None:
    ...
    def test_config_property_defaults_empty(self) -> None:
        cfg = _make_config({"hermes_lark_streaming": {}})
        assert cfg.text_sizes == {}

    def test_config_property_normalizes_mapping(self) -> None:
        cfg = _make_config({
            "hermes_lark_streaming": {
                "text_sizes": {"body": {"mobile": "large"}},
            },
        })
        assert cfg.text_sizes["body"] == {
            "default": "normal",
            "pc": "normal",
            "mobile": "large",
        }
...
class TestGetHermesConfigPath:
    ...
    def test_flush_interval_ms_default(self) -> None:
        """flush_interval_ms 默认 500ms（v1.8: 从 200 上调，避开飞书 5 QPS 上限）."""
        cfg = Config()
        assert cfg.flush_interval_ms == 500.0

    def test_flush_interval_sec_default(self) -> None:
        """flush_interval_sec 默认 0.5 秒（v1.8）."""
        cfg = Config()
