...
class TestBuildStreamingCardV2:
    ...
    def test_unconfigured_text_sizes_preserve_existing_json(self) -> None:
        card = build_streaming_card_v2()
        assert "style" not in card["config"]
        answer = next(
            element
            for element in card["body"]["elements"]
            if element.get("element_id") == "answer_content"
        )
        assert answer["text_size"] == "normal_v2"

    def test_device_text_sizes_emit_cardkit_aliases(self) -> None:
        text_sizes = {
            "body": {"default": "normal", "pc": "small", "mobile": "large"},
            "footer": {"default": "x-small", "pc": "x-small", "mobile": "normal"},
        }
        card = build_streaming_card_v2(text_sizes=text_sizes)
        assert card["config"]["style"]["text_size"] == {
            "hls_body": text_sizes["body"],
            "hls_footer": text_sizes["footer"],
        }
        answer = next(
            element
            for element in card["body"]["elements"]
    ...
    def test_interactive_card_emits_device_aliases_without_streaming_mode(self) -> None:
        text_sizes = {
            "body": {"default": "large", "pc": "normal", "mobile": "large"},
        }
        answer = _streaming_element(content="正文", text_sizes=text_sizes)

        card = build_interactive_card_v2(
            elements=[answer],
            summary="正文",
        ...
        assert card["body"]["elements"][0]["text_size"] == "hls_body"

    def test_scalar_body_size_is_written_directly(self) -> None:
        element = _streaming_element(text_sizes={"body": "large"})
        assert element["text_size"] == "large"

    def test_panel_roles_use_device_aliases(self) -> None:
        panel = build_unified_panel(
            reasoning_rounds=[ReasoningRound(index=1, text="inspect")],
        ...
        nested_text_sizes = [
            child.get("text", {}).get("text_size")
            for child in panel["elements"]
            if isinstance(child.get("text"), dict)
        ]
        assert "hls_reasoning" in nested_text_sizes
        assert "hls_tool" in nested_text_sizes

    def test_notice_and_footer_roles_use_device_aliases(self) -> None:
        mapping = {"default": "x-small", "pc": "small", "mobile": "normal"}
        error_panel = _build_error_panel(
            "failed",
            text_sizes={"notice": mapping},
        )
        footer = _build_footer_elements(
            {},
            text_sizes={"footer": mapping},
        )
        assert error_panel["elements"][0]["text_size"] == "hls_notice"
        assert footer[1]["text_size"] == "hls_footer"
