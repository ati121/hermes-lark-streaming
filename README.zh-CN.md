...
hermes_lark_streaming:
  ...
  # 可选 CardKit 字号；每个角色可使用单值或按设备分别配置。
  text_sizes:
    body:
      default: large               # 旧客户端回退值；优先兼容手机端
      pc: normal                   # PC 端
      mobile: large                # Android 和 iOS 端
    reasoning: small
    tool: x-small
    notice: x-small
    footer:
      default: notation            # 旧客户端回退值；建议与 mobile 保持一致
      pc: x-small
      mobile: notation
  ...
  footer:
    ...
    fields:
      ...
      #   api_calls   — 本轮对话的 API 调用次数，固定显示为 API + 次数（如 API 3）
      #   history_offset — 对话历史偏移量；值越大对话越长，值突然变小说明发生了上下文压缩
      # 每个内层列表为页脚的一行，字段仅在有值时显示
```

`text_sizes` 支持 `body`（正文）、`reasoning`（思考）、`tool`（工具）、
`notice`（提示）和 `footer`（页脚）。设备映射只接受 `default`、`pc`、
`mobile`，缺失设备先继承 `default`，再使用对应角色默认值。可用 CardKit
字号为 `heading-0` 到 `heading-4`、`heading`、`normal`、`notation`、
`xxxx-large`、`xxx-large`、`xx-large`、`x-large`、`large`、`medium`、
`small`、`x-small`。不配置时保持插件当前字号不变。

飞书官方将 `default` 用作无法识别 PC/mobile 差异字号的旧客户端回退值。
若目标是只放大手机端，建议将 `default` 设置为与 `mobile` 相同，并通过 `pc`
显式保持桌面端字号；否则旧版手机端可能回退到桌面字号，看起来像配置没有生效。
