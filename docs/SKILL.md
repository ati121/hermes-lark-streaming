...
**4.7 卡片生命周期 (v1.0.2)**: 4 阶段渐进式卡片构建：Phase 1 用户消息 → 仅创建 "等待上游模型响应" + 加载图标的占位卡片（2 元素）；Phase 2 首个上游模型事件 → 删加载提示、通过 `add_elements` 添加统一面板 + 回答元素；Phase 3 流式更新；Phase 4 完成 → 添加页脚。
...
**卡片生命周期 (4 Phases)**:
- **Phase 1** — 用户发送消息 → 创建占位卡片，仅含"等待上游模型响应" + 加载图标（2 个元素）
