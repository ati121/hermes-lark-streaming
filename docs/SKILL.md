...
| `└ __init__.py` | 注册入口 | `register()`/`unregister()` + 自动备份 config + FeishuClient 预热 + monitor 启动；非 `gateway run` 进程直接跳过 (v1.10)；hook 先于 patch 注册 (v1.11) |
...
**4.22 思考正文折平 (v1.9)**: 预览与展开视图都先经 `_flatten_reasoning_lines` 把模型自己的硬换行折成空格再按宽度截。碎行模型（一行一个短句、`（输出 tool call）` 单独成行）在原来的按宽度取尾逻辑下会变成十几行，200 格宽度预算被行数吃掉。折平后同样 200 格稳定占两行，字数上限与会话快照存的原文都不变；`_align_preview_start` 在折平后仍会向后吸附到原换行处或句末标点，避免从半个词切入。
