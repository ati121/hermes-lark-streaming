...
飞书开放平台为每个 API 请求分配唯一的 `log_id`。当卡片出现 API 报错（如 300313、300317、300305、300309）时，插件日志会自动记录对应的 `log_id`：

```
HLS: unified flush phase 2 batch_update failed: cardkit_batch_update: code=300317, msg=... [log_id=<feishu-log-id>]
```
