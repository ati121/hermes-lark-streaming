...
_TEXT_SIZE_ROLE_ORDER = ("body", "reasoning", "tool", "notice", "footer")
...
def build_card_text_size_style(
    text_sizes: Mapping[str, Any] | None,
) -> dict[str, dict[str, str]]:
    """Build CardKit device aliases for configured mapping-valued roles."""
    if not isinstance(text_sizes, Mapping):
        return {}
    return {
        f"hls_{role}": dict(value)
        for role in _TEXT_SIZE_ROLE_ORDER
        if isinstance((value := text_sizes.get(role)), Mapping)
    }


def _set_text_size(element: dict[str, Any], text_size: str | None) -> dict[str, Any]:
...
def _streaming_element(
    ...
    *,
    ...
    return {
        ...
        "content": content,
        "text_align": "left",
        "text_size": _role_text_size(text_sizes, "body", default="normal_v2"),
...
def _loading_hint_element(status_key: str = "loading_context") -> dict:
    ...
    if status_key not in ("loading_context", "image_analyzing", "context_compressing", "model_thinking"):
    ...
    *,
    ...
    if trimmed_rounds > 0 or trimmed_tools > 0:
        ...
        children.append({
            ...
            "text_size": _role_text_size(
                text_sizes, "notice", default="notation",
            ),
        })
    ...
    if panel_events:
        ...
        for kind, idx in panel_events:
            if kind == "reasoning" and show_reasoning and idx < len(reasoning_rounds):
                ...
                if round_.text.strip():
                    children.append({
                        ...
                        "text": {
                            ...
                            "text_size": _role_text_size(
                                text_sizes, "reasoning", default="notation",
                            ),
                        },
                    })
                ...
                if idx not in rendered_tools:
                    step = tool_steps[idx]
                    children.extend(_build_tool_step_elements(
                        step, text_sizes=text_sizes,
                    ))
        ...
        if current_reasoning_text and show_reasoning:
            ...
            children.append(_build_reasoning_round_title(
                in_progress_idx, 0, finalized=False, text_sizes=text_sizes,
            ))
...
def _build_tool_step_elements(
    ...
    *,
    ...
    elements: list[dict] = [_build_tool_step_title(step, text_sizes=text_sizes)]
    detail = _build_tool_step_detail(step, text_sizes=text_sizes)
    if detail:
        elements.append(detail)
    output = _build_tool_step_output(step, text_sizes=text_sizes)
    if output:
        elements.append(output)
    return elements

def _build_tool_step_title(
    step: dict,
    *,
    text_sizes: Mapping[str, Any] | None = None,
) -> dict:
    ...
    *,
    ...
    return {
        ...
        "text": {
            "tag": "lark_md",
            "content": content,
            "text_size": _role_text_size(text_sizes, "reasoning", default="notation"),
        },
    }
...
def _build_error_panel(
    ...
    *,
    ...
    markdown_el: dict[str, Any] = {
        ...
        "text_size": _role_text_size(text_sizes, "notice", default="notation"),
    }
...
def _build_background_review_panel(
    ...
    *,
    ...
    children: list[dict] = []
    for msg in messages:
        children.append(_set_text_size(
            {"tag": "markdown", "content": msg},
            _role_text_size(text_sizes, "notice", default=None),
        ))
    if not messages:
        children.append({"tag": "markdown", "content": " "})
    panel = _collapsible_panel(
...
def _build_footer_elements(
    ...
    return [
        {"tag": "hr"},
        {
            ...
            "text_size": _role_text_size(text_sizes, "footer", default="notation"),
        },
    ]
