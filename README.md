...
hermes_lark_streaming:
  ...
  # Optional CardKit text sizes. Each role accepts one size or a device mapping.
  text_sizes:
    body:
      default: normal              # Fallback for older clients
      pc: normal                   # Desktop client
      mobile: large                # Android and iOS clients
    reasoning: small
    tool: x-small
    notice: x-small
    footer:
  ...
  footer:
    ...
    fields:
      ...
      #   api_calls   — Number of API calls, always rendered with its label (for example, API 3)
...
`text_sizes` supports the `body`, `reasoning`, `tool`, `notice`, and `footer`
roles. Device mappings accept only `default`, `pc`, and `mobile`; omitted
devices inherit `default`, then the role default. Allowed CardKit sizes are
`heading-0` through `heading-4`, `heading`, `normal`, `notation`,
`xxxx-large`, `xxx-large`, `xx-large`, `x-large`, `large`, `medium`, `small`,
and `x-small`. When omitted, the plugin keeps its existing card sizes.

Feishu's CardKit entity API currently ignores Markdown `text_size` values,
including device aliases. Therefore, setting any `text_sizes` role switches the
card to an ordinary `interactive` message: the plugin sends the complete Card
JSON initially and replaces it with the IM message PATCH API during streaming.
This makes PC/mobile sizes work, while changing the visual update from CardKit's
native typewriter effect to whole-card refreshes (at least 200 ms apart). With no
`text_sizes` configuration, the original CardKit streaming path is unchanged.
